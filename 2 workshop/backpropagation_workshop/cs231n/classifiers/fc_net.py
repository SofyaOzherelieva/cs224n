        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                if i == self.num_layers - 1:
                    input_size = hidden_dims[i - 1]
                    output_size = num_classes
                else:
                    input_size = hidden_dims[i - 1]
                    output_size = hidden_dims[i]

            self.params['W' + str(i+1)] = np.random.normal(loc = 0, scale = weight_scale, size = (input_size, output_size))
            self.params['b' + str(i+1)] = np.zeros(shape = (output_size, ))
            # self.params['gamma' + str(i+1)] = np.ones(shape = (output_size, ))
            # self.params['beta' + str(i+1)] = np.zeros(shape = (output_size, ))
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1:
                ...
                if self.use_dropout:
                    scores, cache_dr = dropout_forward(scores, self.dropout_param)
                    caches['cache_dr' + str(i+1)] = cache_dr
        ...
        for i in range(self.num_layers - 1, -1, -1):
            ...
            if i == self.num_layers - 1:
                dx, grads['W' + str(i+1)], grads['b' + str(i+1)] = affine_backward(dx, cache)
            else:
                if self.use_dropout:
                    cache_dr = caches['cache_dr' + str(i+1)]
                    dx = dropout_backward(dx, cache_dr)
                dx, grads['W' + str(i+1)], grads['b' + str(i+1)] = affine_relu_backward(dx, cache)
